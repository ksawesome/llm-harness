...
from adapters import call_openai_api, call_anthropic_api, call_google_api, call_cohere_api, call_huggingface_api

class ModelConfig(BaseModel):
    name: str = Field(..., description="Model identifier")
    adapter: Callable = Field(..., description="Adapter function")
    rate_limit_seconds: float = Field(default=10.0, description="Minimum seconds between API calls")
    timeout_seconds: int = Field(default=30, description="API call timeout")
    temperature: Optional[float] = Field(default=0.3, description="Override default temperature")
